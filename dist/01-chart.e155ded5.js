...
var margin = { top: 70, left: 125, right: 20, bottom: 100 };
...
var line = d3.line().x(function (d) {
  ...
  return yPositionScale(d.cumulativePoints);
...
function ready(datapoints) {
  ...
  svg.selectAll('.instrument').data(datapoints).enter().append('rect').attr('class', 'instrument').attr('y', function (d) {
    return yPositionScale(d.instrument);
  }).attr('x', 0).attr('height', yPositionScale.bandwidth()).attr('width', 0).attr('fill', 'green').on('mouseover', function (d) {
    div.transition().duration(200).style('opacity', 0.9);
    div.html('<strong>Product:</strong> ' + d.number + '<br/>' + '<br/>' + '<strong>Price:</strong> $' + d.pct_women + '<br/>' + '<br/>' + '<strong>Name on Box:</strong>' + d.NameBox + '<br/>' + '<br/>' + '<strong>App Name:</strong>' + d.App).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px');
  }).on('mouseout', function (d) {
    div.transition().duration(500).style('opacity', 0);
  });

  // Halfway Line
  svg.append('line').attr('class', 'halfway-line').attr('x1', xPositionScale(25)).attr('y1', 0).attr('x2', xPositionScale(25)).attr('y2', height).attr('stroke', 'white').attr('stroke-width', 2);
  ...
  var xAxis = d3.axisTop(xPositionScale).tickValues([10, 25, 50]).tickFormat(function (d) {
    return d;
  }).tickSize(-height);
  ...
  d3.select('#blank-graph').on('stepin', function () {
    ...
    svg.selectAll('.instrument').transition().attr('width', 0);
    svg.selectAll('.y-axis text').attr('fill', '#999');
  });

  d3.select('#intro-graph').on('stepin', function () {
    // Grab all the bars, make them normal
    svg.selectAll('.instrument').transition().attr('width', function (d) {
  ...
  d3.select('#intro-graph').on('stepout', function () {
    svg.selectAll('.y-axis text').transition().attr('font-size', '18');
  });
  ...
  d3.select('#newgraph').on('stepin', function () {
    svg.selectAll('.y-axis text').transition().attr('fill', '#999999').attr('font-size', 18);
  });

  // Second Step
  d3.select('#instruments-mentioned').on('stepin', function () {
    d3.selectAll('.majority-male').style('background', '#E89A98').style('color', 'white');

    svg.selectAll('.instrument-bg').transition().attr('fill', function (d) {
  ...
  d3.select('#instruments-mentioned').on('stepout', function () {
    svg.selectAll('.instrument-bg').transition().attr('fill', '#999999');
  });

  d3.select('#end').on('stepin', function () {
    svg.selectAll('.instrument-bg').transition().attr('fill', '#999999');
  });
  ...
  function render() {
    ...
    svg.selectAll('.instrument').attr('y', function (d) {
      return yPositionScale(d.instrument);
    ...
    svg.selectAll('.gender-label').attr('y', yPositionScale('Flute')).attr('x', function (d) {
      return d === 'Women' ? 10 : newWidth - 10;
...
},{"d3":"..\\node_modules\\d3\\index.js","./data/orchestras.csv":"data\\orchestras.csv"}],"..\\..\\..\\..\\AppData\\Roaming\\npm\\node_modules\\parcel\\src\\builtins\\hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["..\\..\\..\\..\\AppData\\Roaming\\npm\\node_modules\\parcel\\src\\builtins\\hmr-runtime.js","01-chart.js"], null)
//# sourceMappingURL=/01-chart.e155ded5.map